"""Starlink VHF channel DDC built from LFdemod's shared DDC primitives.

Functions and outputs
---------------------
``make_starlink_vhf_decimation_stages(config)``
    Builds the two-stage channel plan; returns two
    ``DecimationStageConfig`` objects.
``downconvert_starlink_vhf_voltage(raw_data, rf_frequency_hz, config, initial_phase)``
    Mixes real voltage to a bounded-band complex channel; returns
    ``(complex64_iq_at_fs_out, final_nco_phase_radians)``.

This module selects a bandwidth suitable for the approximately 41.7 kHz
Starlink VHF LoRa waveform.  It does not perform LoRa synchronization or
interpret packet bytes.
"""
...
from .config import StarlinkVHFDDCConfig


def make_starlink_vhf_decimation_stages(
    config: StarlinkVHFDDCConfig = StarlinkVHFDDCConfig(),
...
def downconvert_starlink_vhf_voltage(
    raw_data: np.ndarray,
    rf_frequency_hz: float,
    config: StarlinkVHFDDCConfig = StarlinkVHFDDCConfig(),
    initial_phase: float = 0.0,
) -> Tuple[np.ndarray, float]:
    """Return one Starlink VHF channel as complex IQ and final NCO phase."""

    first_stage, channel_stage = make_starlink_vhf_decimation_stages(config)
    ...
    for start in range(0, len(raw_data), config.chunk_samples):
        ...
        mixed, phase = frequency_mixing(
            voltage, rf_frequency_hz, config.fs_in, phase
        )
